(function () {
    ...
    if (this.opts.fixWidth) {
      for (let i = 0; i < perNum; i++) {
        ...
        imgList[i].style.left = `${
          imgList[0].offsetLeft + imgList[i].offsetWidth * i
        }px`;
      }
    }
  ...
  proto.getPerNum = function () {
    ...
    return Math.floor(
      parseInt(this.opts.fixWidth || this.opts.width, 10) / singleImgWidth
    );
  };
  ...
  proto.append = function () {
    ...
    while (count < 100) {
      if (
        !tmpObj[imgList[currentPosition - count].offsetLeft] &&
        tmpObj[imgList[currentPosition - count].offsetLeft] !== 0
      ) {
        tmpObj[imgList[currentPosition - count].offsetLeft] =
          imgList[currentPosition - count].offsetLeft;
        perList.push(
          imgList[currentPosition - count].offsetHeight +
            imgList[currentPosition - count].offsetTop
        );
        tmpArr.push(count);
    ...
    for (let i = 0; i < this.opts.number; i++) {
      ...
      imgList[currentPosition + i].style.left = `${
        imgList[currentPosition - tmpArr[Number(pointer)]].offsetLeft
      }px`;
      imgList[currentPosition + i].style.top = `${perList[pointer]}px`;

      perList[pointer] =
        perList[pointer] + imgList[currentPosition + i].offsetHeight; // 数组最小的值加上相应图片的高度
      pointer = this.getMinPointer(perList);
    }
  };
  ...
  proto.done = function () {
    ...
    this.on("done", function () {
      ...
      // 取消加载按钮
      const loadContainer =
        document.getElementsByClassName("waterfall-loading")[0];
  ...
  const scroll = function () {
    ...
    if (!this.opts.scrollElem) {
      ...
      if (
        scrollElemPX + parentElemHeight >
        imgList[imgList.length - 1].offsetTop
      ) {
        isLoading = true;
        this.loadImg();
        this.emit("load");
