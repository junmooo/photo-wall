...
const App: React.FC = () => {
    ...
    if (parent) {
      ...
      for (var i = 0; i < children.length; i++) {
        if (i < cols) {
          ...
          const index = parseInt(String(getMinhIndex(hArr, minH))); //封装一个函数获得最小高度的图片所在列的索引
  ...
  function getChildElement(Parent: HTMLElement, Children: string) {
    ...
    return childrenArr as HTMLDivElement[];
  }

  function getMinhIndex(arr: number[], val: number) {
    for (var i in arr) {
    ...
    <div>
      <div className="waterfull">
        ...
        <div className="box">
          <div className="image">
            <img
              src={`https://api.btstu.cn/sjbz/api.php?lx=dongman&format=images?${Math.random()}`}
              alt="ssss"
            />
          </div>
        </div>
